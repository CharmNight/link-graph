// 资源关系图布局：使用 ELK 分层布局和分区，把节点按泳道（CODE/INTEGRATION/DATA 等）组织。
 ...
 * 资源关系图的 ELK 布局选项。
 *
 * 启用分区约束：让 ELK 按"分区"约束节点位置（每个分区 = 一条泳道）。
 */
 ...
 *
 * 关键策略：
 * - 按泳道把节点分配到不同分区（让同泳道节点在同一行）；
 * - 锚点节点强制放第一层（让用户立即看到锚点）；
 * - 在元数据中保留泳道信息，便于后续渲染层做泳道分隔。
 *
