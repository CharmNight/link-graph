...
class LlmJsonSupportTest {
    ...
    fun parseObjectRejectsTrailingGarbageAndInvalidJson() {
        assertFailsWith<IllegalStateException> {
            LlmJsonSupport.parseObject("""{"ok":true} trailing""")
        }
        assertEquals(null, LlmJsonSupport.parseObjectOrNull("not json"))
    }

    @Test
    fun parseObjectHandlesExponentNumbersAndUnicode() {
        val root = LlmJsonSupport.parseObject("""{"value":1.2e3,"text":"\uD83D\uDE00"}""")
    ...
    fun parseObjectRejectsRawControlCharacters() {
        assertFailsWith<IllegalStateException> {
            LlmJsonSupport.parseObject("{\"bad\":\"\u0001\"}")
        }
    }

    @Test
    fun jsonObjectAccessorsPreserveNestedContent() {
        val root = LlmJsonSupport.parseJsonObject("""{"choices":[{"message":{"content":"hello"}}]}""")
    ...
    fun serializeObjectBuildsValidJson() {
        val payload = LlmJsonSupport.toJson(
            linkedMapOf(
                "model" to "gpt",
                "messages" to listOf(
        ...
        val parsed = LlmJsonSupport.parseObject(payload)
