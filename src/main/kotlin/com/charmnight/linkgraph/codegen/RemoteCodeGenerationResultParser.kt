...
internal object RemoteCodeGenerationResultParser {
    ...
    fun parse(
        ...
        /** 远程返回的代码草稿列表。 */
        val drafts = (root["drafts"] as? List<*>).orEmpty().mapNotNull { parseDraft(it as? Map<*, *>) }
    ...
    /** 解析单个代码草稿对象。 */
    private fun parseDraft(raw: Map<*, *>?): GeneratedCodeDraft? {
        raw ?: return null
        /** 草稿目标路径。 */
        val targetPath = raw["targetPath"] as? String ?: return null
        /** 草稿标题，缺失时回退到文件名。 */
