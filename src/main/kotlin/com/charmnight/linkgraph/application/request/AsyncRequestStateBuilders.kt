...
/**
 * AsyncRequestLifecycleSupport 的纯状态构造 helper（P2-1 深度拆分）。
 *
 * 把 buildAsyncRequestLifecycleResult / buildTimedOutRequestState /
 * buildSucceededRequestState / buildFailedRequestState 这 4 个无状态构造函数抽出来，
 * 与 AsyncRequestLifecycleSupport 的 tracker 调度 / 异步派发主流程解耦后便于复用与单独测试。
 */

/**
 * 构造异步请求启动时的展示信息（AsyncRequestLifecycleResult）。
 *
 * 综合设置推导 executionMode（REMOTE_READY / LOCAL_RULE / DISABLED / REMOTE_FALLBACK）、
 * 流式支持、超时、provider label、endpoint 摘要等，并据此生成对应的 statusMessage / detailMessage。
 */
internal fun buildAsyncRequestLifecycleResult(
    requestId: Long,
    sceneLabel: String,
    settings: LinkGraphSettingsState,
    timeoutOverrideMillis: Long?,
    disabledMode: AsyncRequestExecutionMode = AsyncRequestExecutionMode.LOCAL_RULE,
    ...
    val executionMode = when {
        ...
        else -> AsyncRequestExecutionMode.LOCAL_RULE
    }
    val remoteRequested = executionMode == AsyncRequestExecutionMode.REMOTE_READY
    val timeoutMillis = (timeoutOverrideMillis
        ?: sanitized.effectiveTimeoutSeconds().toLong() * 1_000L)
        .coerceAtLeast(1L)
    val timeoutSecondsText = ((timeoutMillis + 999L) / 1_000L).toString()
    val requestState = AsyncRequestState.running(
        requestId = requestId,
        scene = sceneLabel,
        executionMode = executionMode,
        statusMessage = when (executionMode) {
            AsyncRequestExecutionMode.REMOTE_READY -> "正在等待远程 LLM ${sceneLabel}响应"
            AsyncRequestExecutionMode.LOCAL_RULE -> "正在执行${sceneLabel}本地规则"
            AsyncRequestExecutionMode.DISABLED -> "正在处理${sceneLabel}请求"
            AsyncRequestExecutionMode.REMOTE_FALLBACK -> "正在执行${sceneLabel}"
        },
        detailMessage = when (executionMode) {
            AsyncRequestExecutionMode.REMOTE_READY ->
            ...
            AsyncRequestExecutionMode.LOCAL_RULE -> when {
            ...
            AsyncRequestExecutionMode.DISABLED ->
                "LLM 生成功能未启用，当前不会发起远程请求。"
            AsyncRequestExecutionMode.REMOTE_FALLBACK ->
                "当前请求已回退到本地规则。"
        },
        streaming = executionMode == AsyncRequestExecutionMode.REMOTE_READY && streamingSupported,
        streamPhase = if (executionMode == AsyncRequestExecutionMode.REMOTE_READY && streamingSupported) {
            "STREAMING"
        } else {
            null
        },
...
/** 根据请求展示信息构造超时终态（AsyncRequestState.timedOut）。 */
internal fun buildTimedOutRequestState(
    presentation: AsyncRequestLifecycleResult,
): AsyncRequestState {
    val timeoutSecondsText = ((presentation.timeoutMillis + 999L) / 1_000L).toString()
    return AsyncRequestState.timedOut(
        message = when (presentation.executionMode) {
            ...
            AsyncRequestExecutionMode.LOCAL_RULE ->
...
/** 根据请求展示信息构造成功终态（AsyncRequestState.succeeded）。 */
internal fun buildSucceededRequestState(
    presentation: AsyncRequestLifecycleResult,
    ...
    val fallbackUsed = executionMode == AsyncRequestExecutionMode.REMOTE_FALLBACK
    val detailMessage = when {
        ...
        executionMode == AsyncRequestExecutionMode.DISABLED ->
            "LLM 生成功能已关闭，当前结果用于说明为何本次请求没有发起远程调用。"
        executionMode == AsyncRequestExecutionMode.LOCAL_RULE && !presentation.llmEnabled ->
            "远程 LLM 未启用，当前结果来自本地规则或模板执行。"
        executionMode == AsyncRequestExecutionMode.LOCAL_RULE && presentation.remotePresetSelected ->
            "远程配置未就绪，当前结果来自本地规则或模板执行。"
        else -> "当前结果来自本地规则或模板执行。"
    }
    return AsyncRequestState.succeeded(
        requestId = presentation.requestId,
        scene = presentation.sceneLabel,
        executionMode = executionMode,
        statusMessage = when {
            fallbackUsed -> "${presentation.sceneLabel}完成，已回退到本地规则或模板结果。"
            executionMode == AsyncRequestExecutionMode.LOCAL_RULE ->
...
/** 根据请求展示信息构造失败终态（AsyncRequestState.failed）。 */
internal fun buildFailedRequestState(
    presentation: AsyncRequestLifecycleResult,
    message: String,
    detailMessageOverride: String? = null,
): AsyncRequestState = AsyncRequestState.failed(
    message = message,
    requestId = presentation.requestId,
    scene = presentation.sceneLabel,
    executionMode = presentation.executionMode,
