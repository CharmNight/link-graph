 ...
 * 从 WorkflowComposition 抽出的独立 object，负责创建语义分析链路的共享组件：
     * - 主体句柄工厂 CodeSubjectHandleFactory
...
internal object CompositionSharedInfrastructure {
     ...
     */
    fun resolveSubjectLocator(runtimeHooks: LinkGraphProjectRuntimeHooks): SubjectLocator =
        runtimeHooks.subjectLocator ?: createDefaultSubjectLocator()

    fun resolveSemanticAnalyzer(
        runtimeHooks: LinkGraphProjectRuntimeHooks,
        project: Project,
        logger: Logger,
        infrastructure: InfrastructureComposition,
    ): SemanticAnalyzer =
        runtimeHooks.semanticAnalyzer ?: createDefaultSemanticAnalyzer(project, logger, infrastructure)

    fun resolveAnalysisOutcomeFactory(
        runtimeHooks: LinkGraphProjectRuntimeHooks,
        infrastructure: InfrastructureComposition,
    ): AnalysisOutcomeFactory =
        runtimeHooks.analysisOutcomeFactory ?: createDefaultAnalysisOutcomeFactory(infrastructure)
}
