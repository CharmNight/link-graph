 ...
 *
 * - [BootstrapPayloadDto]：完整快照根 DTO，前端启动 JSON 顶层结构
 * - [ArtifactSlicePayloadDto] / [FeedbackSlicePayloadDto]：增量切片 DTO
     * - 信封 DTO：[SnapshotEnvelopePayloadDto] / [ArtifactSliceEnvelopePayloadDto] / [FeedbackSliceEnvelopePayloadDto]：
 *   信封外层包装（前端 event.detail 结构）
 * - [ArtifactContentsSliceDto]：按需产物拉取响应
 * - 工件摘要 + 操作反馈 / Mermaid 问题 / Diff 条目 / 同步预览等运行时 DTO
 *
 * 信封状态字段统一走 [TransportSliceStateDto] 密封类型，与 [GraphEditorTransportEnvelope] 对齐。
 */
...
/**
 * Bootstrap 序列化的根 DTO（前端启动 JSON 顶层结构）。
 *
...
/**
 * 产物增量切片 DTO（前端按产物变化接收的局部更新）。
 *
 * 所有字段可选：只承载发生变化的字段（与上一版产物引用对比决定）。
 ...
 *
 * 当本快照与上一快照只有反馈文案变化时使用，避免下发完整启动载荷。
 */
 ...
 * 状态字段持有 [TransportSliceStateDto] 的具体子类型。
 */
...
/**
 * 按需产物拉取响应 DTO（[GraphBrowserTransportDispatcher] 用）。
 *
 * 与 [ArtifactSlicePayloadDto] 不同：这是前端主动拉取产物内容时的最小响应，
