...
Link Graph 是一个 IntelliJ Platform 插件，用于在项目内查看代码与资源之间的关联关系。它从当前方法或资源出发构建图数据，在工具窗口中展示事实图、流程图和资源关系视图，并围绕这张图提供 Mermaid 导入导出、步骤化讲解、多轮问答、草稿确认、实现计划和代码草稿等能力。

## 项目定位

这个项目不是独立桌面程序，也不是通用可视化平台，而是一个运行在 IntelliJ IDEA 内部的开发辅助插件。它的目标是把“当前方法、相关调用链、配置资源、流程图意图、草稿确认和代码草稿”收敛到同一个工作台里，降低阅读和修改复杂链路时的上下文切换成本。

工作台的核心产品链路是：

`流程图 / 代码双锚点 -> 讲解 / 问答 -> 草稿 -> 计划 -> 代码`
...
- 从图节点跳回源码
- 支持本地规则与远程 LLM 配置的讲解、问答、草稿确认、计划和代码草稿流程
- 通过受控 Agent Runtime 驱动问答、计划与代码生成流程
- 通过 Tool Facade 按需读取图、代码、草稿和校验结果
- 通过 Artifact 在问答、草稿、计划与代码阶段之间传递结构化结果
...
- 执行模型
  - 用户的一次问答、计划生成或代码生成请求，会启动一次受控 Agent Run。
...
- 回退路径
  - 当远程模型不可用时，问答、计划和代码流程仍可回退到本地规则或模板结果。
