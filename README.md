# Link Graph

Link Graph 是一个 IntelliJ Platform 插件，用于在项目内查看代码、资源、架构索引和变更影响之间的关联关系。它从当前方法或资源出发构建主体链路图，也可以加载项目级架构图、类图和 Review Graph，并围绕这些图提供 Mermaid 导入导出、步骤化讲解、多轮问答、草稿确认、实现建议和代码 diff 等能力。

## 项目定位

这个项目不是独立桌面程序，也不是通用可视化平台，而是一个运行在 IntelliJ IDEA 内部的开发辅助插件。它的目标是把”当前方法、相关调用链、配置资源、流程图意图、草稿确认和代码 diff”收敛到同一个工作台里，降低阅读和修改复杂链路时的上下文切换成本。

工作台的核心产品链路是：

`流程图 / 代码双锚点 -> 理解链路 / 核验证据 / 风险问答 -> 草稿确认 -> 实现建议 -> 代码 diff / 写回`

## 当前能力范围

- 基于 Kotlin 与 IntelliJ Platform Gradle Plugin 构建的 IDEA 插件
- 通过 JCEF 承载内嵌 React 前端
- 六类工作区视图
  - 事实图
  - 流程图
  - 资源关系视图
  - 架构图
  - 类图
  - Review Graph
- Mermaid 导入、导出、校验与差异流程
- 从图节点跳回源码
- 支持本地规则与远程 LLM 配置的讲解、问答、草稿确认、实现建议和代码 diff 流程
- 通过受控 Agent Runtime 驱动问答、实现建议与代码 diff 流程
- 通过 Tool Facade 按需读取图、代码、草稿和校验结果
- 通过 Artifact 在问答、草稿、实现建议与代码阶段之间传递结构化结果
- 问答支持 `Auto / 只回答 / 风险复核 / 代码调整` 模式，后端会回传请求模式与实际执行模式
- 问答请求页可以查看本轮提示词、实际附带源码和取证轨迹

## 文档导航

- [快速开始](docs/getting-started.md)
- [使用说明](docs/usage.md)
- [开发说明](docs/development.md)
- [架构说明](docs/architecture.md)
- [项目结构说明](docs/project-structure.md)
- [功能与限制](docs/features-and-limitations.md)

## 仓库结构

- `src/main/kotlin`：插件后端实现
- `src/main/resources`：插件描述与文案资源；构建时会合入生成的前端资源
- `src/test/kotlin`：单元测试与平台测试
- `src/test/resources/fixtures`：后端测试样例
- `src/integrationTest/kotlin`：集成测试
- `web/`：React + Vite 前端工作区
- `docs/project-structure.md`：目录结构说明
- `docs/architecture.md`：完整架构说明

## 快速上手

1. 安装依赖：`npm --prefix web ci`
2. 运行后端测试：`./gradlew test`
3. 启动插件沙箱：`./gradlew runIde`
4. 在沙箱 IDE 中打开一个 Java 或 Kotlin 项目
5. 通过编辑器右键菜单或工具菜单打开 Link Graph

更详细的步骤见 [docs/getting-started.md](docs/getting-started.md)。
实际功能操作说明见 [docs/usage.md](docs/usage.md)。

## 环境要求

- JDK 17
- 可运行当前前端工具链的 Node.js 与 npm
- IntelliJ IDEA 2023.3.4 或更高兼容版本；最低平台 build 由 `gradle.properties` 中的 `platformSinceBuild` 声明

## 常用命令

```bash
./gradlew test
./gradlew integrationTest
./gradlew check
./gradlew runIde
npm --prefix web test
```

`./gradlew check` 会执行后端检查和前端测试。`./gradlew runIde` 会启动带插件沙箱的 IDE。

## 核心工作流

- 当前主体链路加载
  - 从当前光标所在的方法或资源主体出发生成事实图，并投影为不同视图。
- 图谱视图阅读
  - 在事实图、流程图、资源关系视图之间阅读当前主体链路。
  - 按需加载项目级架构图、类图和 Review Graph，分别观察项目结构、类型关系和变更影响面。
- Mermaid 设计流
  - 导入 Mermaid、校验问题、对比现有工作图，并生成同步预览。
- 工作台协同
  - 顶部任务栏提供 `理解链路 / 核验证据 / 风险问答 / 草稿确认 / 代码落地` 五阶段流程。
  - 左侧链路大纲从入口、关键路径、资源证据、风险线索和草稿影响定位图节点。
  - 中间图谱舞台提供事实图、流程图、资源关系视图、架构图、类图和 Review Graph，图谱模式与工作流阶段互不绑定。
  - 项目级架构图、类图和 Review Graph 都是基于当前范围、锚点和可读窗口生成的投影，不表示把项目、三方库和 JDK 全量节点一次性铺满画布。
  - 架构图默认优先展示项目源码、资源和聚合关系；包依赖视图或显式开关会纳入三方库和 JDK 依赖分组。
  - 类图可以从当前编辑器类打开，也可以从架构节点下钻；无显式范围时会以当前编辑器类作为可解释锚点。
  - Review Graph 以当前 diff 或工作区变更为输入，选择部分 diff 条目时会按选择范围生成影响面。
  - 右侧阶段工作台按当前阶段展示讲解、证据、问答、草稿或代码 diff，并在草稿到代码之间承接实现建议。
  - 底部变更托盘汇总候选变更、已确认草稿、阻塞风险、diff 状态和写回入口。
  - 讲解输出稳定步骤，问答按模式隔离普通回答、风险复核、代码调整和风险线程取证，草稿是统一业务真相层。
  - 问答请求状态会展示请求模式、实际模式、提示词、实际附带源码和取证轨迹。
- 实现建议与代码 diff
  - 从已确认草稿层推导实现建议，再生成可写回工程的代码 diff，并支持批量或单条写回。
- 源码导航
  - 从图节点、资源节点或草稿结果回跳到源文件位置。

## LLM 相关说明

插件同时支持本地规则执行和远程 LLM 提供方。远程能力不是默认必需项，需要在插件设置中显式配置。远程执行未启用、配置不完整或调用失败时，部分流程会回退到本地规则或模板结果，而不会伪装成远程请求成功。

LLM 层的核心设计如下：

- 执行模型
  - 用户的一次问答、实现建议生成或代码 diff 请求，会启动一次受控 Agent Run。
  - Agent Run 在预算、停止条件和安全边界约束下多步执行，而不是无约束自由对话。
- 上下文获取
  - Agent 不依赖一次性大 prompt 读取全部信息，而是通过 Tool Facade 按需读取图节点、代码片段、草稿状态和验证结果。
  - 问答会先解析投影节点、当前视图、canonical 节点和源码锚点，再按预算收集方法源码、类上下文、imports 和邻近证据。
  - 跨阶段共享依赖结构化 Artifact，而不是超长聊天历史。
- 产品边界
  - 问答默认 `Auto`，后端会识别实际模式；显式模式会被优先尊重。
  - `只回答` 不推进候选变更或风险线程，`风险复核` 不自动推进草稿，`代码调整` 才允许保留候选变更，风险线程继续取证固定走取证模式。
  - 流程图意图优先于代码事实。
  - 草稿确认是正式意图入口。
  - 代码 diff 生成不等于直接写回源码。
  - 写回工程仍然需要经过显式确认和写回边界控制。
- 回退路径
  - 当远程模型不可用时，问答、实现建议和代码 diff 流程仍可回退到本地规则或模板结果。
  - 回退结果会明确标识来源，不会伪装成远程模型成功。

当前与 LLM 相关的功能边界和限制见 [docs/features-and-limitations.md](docs/features-and-limitations.md)。

## 当前状态

仓库仍处于持续演进阶段。公开文档以当前实现和工作台语义为准，对外描述统一围绕图工作台、Agent Runtime、草稿确认边界和代码 diff 链路展开。

## 参与贡献

见 [CONTRIBUTING.md](CONTRIBUTING.md)。

## 安全说明

见 [SECURITY.md](SECURITY.md)。

## 许可证

本项目采用 MIT 许可证，见 [LICENSE](LICENSE)。
