...
- 问答支持 `Auto / 只回答 / 审计风险 / 代码调整` 模式，后端会回传请求模式与实际执行模式
...
- 工作台协同
  ...
  - 中间图谱舞台继续提供事实图、流程图和资源关系视图，图谱模式与工作流阶段互不绑定。
  - 右侧阶段工作台按当前阶段展示讲解、证据、问答、草稿或代码 diff，并在草稿到代码之间承接实现建议。
  - 底部变更托盘汇总候选变更、已确认草稿、阻塞风险、diff 状态和写回入口。
  - 讲解输出稳定步骤，问答按模式隔离普通回答、审计风险、代码调整和风险线程取证，草稿是统一业务真相层。
...
- 产品边界
  ...
  - `只回答` 不推进候选变更或风险线程，`审计风险` 不自动推进草稿，`代码调整` 才允许保留候选变更，风险线程继续取证固定走取证模式。
  - 流程图意图优先于代码事实。
  - 草稿确认是正式意图入口。
  - 代码 diff 生成不等于直接写回源码。
