...
- `workbench/` 负责审计会话、风险线程、草稿确认、步骤详情和讨论面板等工作台域界面
...
- 前端不传模式时默认为 `Auto`
- `Auto` 会由后端根据问题文本和风险线程来源识别为普通回答、风险审计、代码调整或风险线程取证
- 显式选择的问答模式会被优先尊重
- 风险线程继续取证固定进入风险线程取证流程

不同模式有硬边界：普通回答不会保留候选变更或风险线程；风险审计允许风险线程但不自动推进草稿；代码调整才保留证据充分的候选变更；风险线程取证只更新目标线程。
