...
- 应用服务与工作流层
  ...
  - `application/event` 和 `application/result` 只承载应用事件与应用结果；展示文案由 UI presenter 负责，视图投影由 `projection/business` 和 `semantic/outcome` 的 projector/result 模型负责。
- 分析与图领域层
  ...
  - `projection` 提供统一图窗口裁剪、隐藏计数、overflow 元数据、交互式投影内核和各视图业务 projector，避免各视图重复实现可见窗口逻辑。
