...
- 应用服务与工作流层
  - `GraphEditorApplicationService` 负责承接 IDE 动作、工具窗口 bridge 和调试自动化请求，并组织链路加载、问答、讲解、实现建议、代码 diff、同步预览和导航等流程。
- 分析与图领域层
...
- 前端不传模式时默认为 `Auto`
- `Auto` 会由后端根据问题文本和风险线程来源识别为普通回答、审计风险、代码调整或风险线程取证
- 显式选择的问答模式会被优先尊重
- 风险线程继续取证固定进入风险线程取证流程

不同模式有硬边界：普通回答不会保留候选变更或风险线程；审计风险允许风险线程但不自动推进草稿；代码调整才保留证据充分的候选变更；风险线程取证只更新目标线程。
